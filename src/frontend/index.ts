...
document.addEventListener('DOMContentLoaded', async () => {
    app.init({
        ...
        const lastCrashes = document.getElementById("lastCrashes");
        const snackbar = document.getElementById("snackbar");
        if (!cp || !balance || !betButton || !betInput || !lastCrashes || !snackbar) return;
        cp.appendChild(app.canvas);
        main(app, balance, betButton, betInput, lastCrashes, snackbar);
    });
});